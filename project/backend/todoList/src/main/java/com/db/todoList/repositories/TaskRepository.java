...
public interface TaskRepository extends JpaRepository<Task, Long> {
    ...
    @Query("SELECT t FROM Task t WHERE t.project.projectId = :projectId AND t.dueDate = CURRENT_DATE")
    List<Task> findTasksDueToday(@Param("projectId") Long projectId);
    
    // Find overdue tasks
    @Query("SELECT t FROM Task t WHERE t.project.projectId = :projectId AND t.dueDate < CURRENT_DATE AND t.status != 'COMPLETED'")
